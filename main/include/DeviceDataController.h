...
struct DeviceData{
    ...
    char* serialize() const {
        ...
        char* jsonStr = cJSON_PrintUnformatted(root);  // or cJSON_Print() for pretty
    ...
    bool isNewer(const DeviceData& other) const  {
        ...
        if (thisSec < otherSec && (otherSec - thisSec) > 12 * 3600)
            return true;

        // If this is more than 12 hours "later" than other, assume other wrapped
        if (thisSec > otherSec && (thisSec - otherSec) > 12 * 3600)
            return false;
